...
exports.uploadCourse = (0, catchAsyncErrors_1.default)(async (req, res, next) => {
    try {
        ...
        if (!demoVideo)
            return next(new errorHandler_1.default("Upload Course Intro Video", 403));
        if (!data)
        ...
        // for image upload
        if (!thumbnail)
            return next(new errorHandler_1.default("Upload Course Image", 403));
        ...
        if (thumbnail) {
            ...
            //
            await cloudinary_1.default.upload(thumbnail.filepath, {
                folder: folderPath,
                transformation: {
                    gravity: "face",
                },
        ...
        if (Array.isArray(demoVideo))
            return next(new errorHandler_1.default("Multiple videos not allowed", 422));
        // 2. Check if the file is a video (not an image)
        if (!demoVideo.mimetype?.startsWith("video/")) {
            return next(new errorHandler_1.default("Invalid format. File must be a video (e.g., .mp4, .mov)", 400));
        }
        // 3. Validate file size (e.g., 100MB limit)
        const MAX_SIZE_BYTES = 30 * 1024 * 1024; // 100MB
        if (demoVideo.size > MAX_SIZE_BYTES) {
            return next(new errorHandler_1.default(`Video exceeds ${MAX_SIZE_BYTES / (1024 * 1024)}MB limit`, 413 // Payload Too Large
            ));
        }
        ...
        if (demoVideo) {
            ...
            // delete old video
            await cloudinary_1.cloudApi.delete_resources_by_prefix(folderPath);
            await cloudinary_1.default.upload(demoVideo.filepath, {
                folder: folderPath,
                resource_type: "video",
                chunk_size: 6000000,
